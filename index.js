...
const verifyToken = async (req, res, next) => {
  try {
    ...
    if (!session) {
      console.log("❌ Error: Token not found in MongoDB session collection!");
      return res.status(401).send({ message: "unauthorized access" });
    }
...
app.get("/", (req, res) => {
  res.send("epilogue lab server is running perfectly!");
});
...
app.get("/api/admin/stats", verifyToken, verifyAdmin, async (req, res) => {
  try {
    ...
    const stats = {
      ...
      premiumUsers: await usersCollection.countDocuments({ isPremium: true }),
...
app.get("/api/lessons", async (req, res) => {
  try {
    ...
    const sortObj =
      req.query.sort === "mostSaved" ? { likesCount: -1 } : { createdAt: -1 };
...
app.get("/api/favorites", verifyToken, async (req, res) => {
  try {
    ...
    const result = favorites.map((fav) => ({
      ...fav,
      lesson: lessons.find((l) => l._id.toString() === fav.lessonId.toString()),
    }));
